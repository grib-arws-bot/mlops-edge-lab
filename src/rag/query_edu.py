...
# 2026-09-19: "여러 근거 중 질문과 무관한 건 무시하라"는 지시를 여기 추가해봤지만
# 효과가 없었다 — 4B급 소형 생성 모델이 그 판단을 못 해서, 오히려 무관한 내용을
# 질문에 억지로 갖다붙이는 더 나쁜 답변이 나왔다(실측 확인). 되돌림 — 진짜 해결책은
# 검색 단계(재순위화·하이브리드 검색 등)에서 찾아야 한다는 게 이번에 확인된 것.
