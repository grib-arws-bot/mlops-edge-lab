...
deck.add_body_table(
    headerSub="2-2. 센서 데이터",
    headline="3개 카테고리, 임계값은 실제 법령 기준으로 교체",
    rows=[
        ["카테고리", "측정 물질", "단위 예", "임계값 근거", "비고"],
        ["공기질", "온도·습도·미세먼지·CO2·TVOC", "℃/%/㎍/m³/ppm", "KOSHA·실내공기질관리법", "6개 물질"],
        ["가스", "산소(O2)·수소", "%/%LEL", "산업안전보건기준 제618조", "질식 위험 중심"],
        ["조리흄", "포름알데히드 등 9종", "ppm/ng·mg/m³", "고용노동부 노출기준 고시", "4종은 근사값"],
        ["구조 변경", "헬륨·질소·아르곤 → 산소(O2) 통합", "%", "단순 질식제라 개별기준 없음", "산소결핍 18% 미만"],
    ],
)
...
deck.add_body_3level(
    headerSub="4. AI 추천",
    headline="아직 미구현 — 원칙이 통할지에 대한 사고 실험",
    items=[
        {"text": "목적 — 추천 이유를 설명 가능한 AI"},
        {"text": "판정/추천은 규칙·통계 모델, 설명은 LLM"},
        {"text": "이 프로젝트의 핵심 원칙과 자연스럽게 겹침"},
        {"text": "\"근거 없으면 모른다\"가 설명가능AI 요구와 일치"},
        {"text": "실제 구현된 것은 전혀 없음(가설 단계)"},
        {"text": "단계별 변경(가설)"},
        {"text": "데이터 = 사용자 행동 로그, RAG = 상품 카탈로그"},
        {"text": "cold-start·대규모 협업필터링은 검토 안 됨"},
        {"text": "Agent = 추천 스코어 인용 후 이유 설명"},
        {"text": "구현 상태 — 미구현(사고 실험)"},
    ],
)
...
out_path = r"D:\Code-CLI\MLOps-Edge-Lab\docs\교육자료\MLOps-Edge-Lab_교육자료_v4.pptx"
deck.save(out_path)
